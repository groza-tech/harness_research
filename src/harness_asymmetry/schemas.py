...
#: Допуск при проверке цены на попадание в ``[c, v]``, в рублях.
#:
#: Резервные величины разыгрываются непрерывно (v = 1 425 360.87), а модели
#: называют цену в целых рублях. Без допуска каждое такое округление вверх
#: засчитывается как нарушение бюджетного ограничения — то самое, по которому
#: Годе и Сандер (1993) отличают ZI-U от ZI-C. В прогоне 16.08.2026 так набрало
#: 6% «нарушений», и все они были меньше 50 копеек. Один рубль — квант ценовой
#: сетки: превышение внутри кванта есть округление, а не согласие торговать
#: хуже собственной резервной величины.
